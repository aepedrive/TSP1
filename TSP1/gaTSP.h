...
class CgaTSP
{
	...
	public:
		...
		CgaTSP(double mut_rat, double cross_rat, int pop_size, int NumCities, int map_width, int map_height):
		  m_dMutationRate(mut_rat), m_dCrossOverRate(cross_rat), m_iPopSize(pop_size),
			  m_iFittestGenome(0), m_iGeneration(0), m_dShortestRoute(999999999),
			  m_dLongestRoute(0), m_iChromoLength(NumCities), m_bStarted(false)
		  {
			  //set up the map
