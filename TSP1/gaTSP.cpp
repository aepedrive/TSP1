...
// defined in the text
//-------------------------------------------------------------------
void CgaTSP::CrossoverPMX(const vector<int> &mum, const vector<int> &dad,
...
void CgaTSP::Render(HDC surface, int cx, int cy)
{
	...
	if(m_iGeneration != 0)
	{
		MoveToEx(surface, (int)m_pMap->m_vecCityCoOrds[route[0]].x,
			(int)m_pMap->m_vecCityCoOrds[route[0]].y, NULL);
		...
		for(int i = 1; i < route.size(); ++i)
		{
			...
			if(NUM_CITIES < 100)
			{
				...
				TextOutA(surface, CityX, CityY, CityNumber.c_str(), CityNumber.size());
			}
		}
	...
	TextOutA(surface, 5, 5, sGen.c_str(), sGen.size());

	if(!m_bStarted)
	{
		string Start = "Press Return to start a new run";
		TextOutA(surface, cx / 2 - (Start.size() * 3), cy - 20, Start.c_str(), Start.size());
	}
	else
	{
